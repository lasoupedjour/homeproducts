...
@Component({
    selector: 'resumen-de-refacciones',
    templateUrl: './resumen-de-refacciones.component.html'
})
export class ResumenDeRefaccionesComponent {
    title = 'app';
    ...
    filtrarReporte() {
        ...
        if(this._global.user.nivel=="administrador")
          cds   = this.filterForm.controls.Cds.value;
        else
          cds = this._global.user.IDCentro;

        if(cds!="" ){
          var params = {};
          if(this._global.user.nivel=="administrador")
            pais   = this.filterForm.controls.Pais.value;
          else
            pais = this._global.user.Pais;

          if(this._global.user.nivel=="administrador")
            master   = this.filterForm.controls.Master.value;
          else
            master = this._global.user.IDMaster;
          ...
          this.subscription = this._httpService.postJSON(params, 'administracion/filtrar-resumen-de-refacciones.php')
              .subscribe(
              data => {
